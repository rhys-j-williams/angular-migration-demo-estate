# Vault policy: txn-posting-service
# Owner: GIS Secrets Management (platform-tooling CODEOWNERS routes this to gis-appsec).
# Bound to the Kubernetes auth role "txn-posting-service" in the cswt-{dev,uat,prod} namespaces via
...
path "secret/data/cswt/{{identity.entity.aliases.auth_kubernetes_cswt.metadata.service_account_namespace}}/txn-posting-service/*" {
  capabilities = ["read"]
}

path "secret/metadata/cswt/{{identity.entity.aliases.auth_kubernetes_cswt.metadata.service_account_namespace}}/txn-posting-service/*" {
...
path "database/creds/txn-posting-service-{{identity.entity.aliases.auth_kubernetes_cswt.metadata.service_account_namespace}}" {
  capabilities = ["read"]
}
